...
#define CPY(_s, _l) { if (dest + (_l) >= ph->bufsize - 1) { dest_bufend = dest; dest = 0; } \
    ...
    while (i < end) {
        ...
        case '\r':
            CPY(s, i - s);
            if (!memcmp(ph->buffer + lastmod_s, ph->buffer + i + 1, lastmod_len * sizeof(Py_UCS4))) i += lastmod_len;
            s = i + 1;
            break;
        case '\x1b':
        ...
        if (x == xnum) {
            ...
            if (!(ph->buffer[i] == '\x1b' && ph->buffer[i+1] == '[') && lastmod_len) CPY(lastmod_s, lastmod_len);
        }
    }
